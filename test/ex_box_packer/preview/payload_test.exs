defmodule ExBoxPacker.Preview.PayloadTest do
  ...
  defp box(ref, s), do: %SimpleBox{reference: ref, outer_width: s, outer_length: s, outer_depth: s,
                                   empty_weight: 0, inner_width: s, inner_length: s, inner_depth: s, max_weight: 1_000_000}
  defp item(desc, d), do: %SimpleItem{description: desc, width: d, length: d, depth: d, weight: 1, allowed_rotation: :best_fit}
  ...
  test "build/1 produces the items+boxes shape with placement-ordered items" do
    ...
    assert Enum.all?(placed, fn [idx, _x, _y, _z, w, l, d] -> idx == 0 and [w, l, d] == [5, 5, 5] end)
  end
